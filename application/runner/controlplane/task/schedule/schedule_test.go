...
func TestScheduler_On(t *testing.T) {
	...
	t.Run("a service of a stack goes where its stack is", func(t *testing.T) {
		...
		stacks.On("GetOne", mock.Anything, "stack-uuid").
			Return(stack.Stack{UUID: "stack-uuid", Slug: "shop-abcde", NodeName: "runner-orchestrator-01"}, nil).Once()
		...
		require.NoError(t, New(&stacks, &producer).On(context.Background(), &service, "runner-orchestrator-03", 0))

		event := scheduled(t, &producer)
		assert.Equal(t, "runner-orchestrator-01", event.NominatedNode)
	...
	t.Run("a task of its own goes where it is asked to", func(t *testing.T) {
		...
		require.NoError(t, New(&stacks, &producer).On(context.Background(), &standalone, "runner-orchestrator-03", 2))

		event := scheduled(t, &producer)
		assert.Equal(t, "runner-orchestrator-03", event.NominatedNode)

...
func TestUseCase_Execute(t *testing.T) {
	...
	t.Run("kills the containers a task is running", func(t *testing.T) {
		t.Parallel()
		...
		containerManager.On("Kill", mock.Anything, "container-1").Return(nil).Once()
		containerManager.On("Kill", mock.Anything, "container-2").Return(nil).Once()
	...
	t.Run("a task with no container on this node is not this node's to kill", func(t *testing.T) {
		t.Parallel()
		...
		containerManager.AssertNotCalled(t, "Kill", mock.Anything, mock.Anything)
	})

	t.Run("a container that will not die is reported", func(t *testing.T) {
		t.Parallel()
		...
		containerManager.On("Kill", mock.Anything, "container-1").Return(expected).Once()

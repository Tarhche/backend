...
func TestUseCase_Execute(t *testing.T) {
	...
	t.Run("kills the tasks a task is running", func(t *testing.T) {
		t.Parallel()
		...
		taskManager.On("Kill", mock.Anything, "task-1").Return(nil).Once()
		taskManager.On("Kill", mock.Anything, "task-2").Return(nil).Once()
	...
	t.Run("a task with no task on this node is not this node's to kill", func(t *testing.T) {
		t.Parallel()
		...
		taskManager.AssertNotCalled(t, "Kill", mock.Anything, mock.Anything)
	})

	t.Run("a task that will not die is reported", func(t *testing.T) {
		t.Parallel()
		...
		taskManager.On("Kill", mock.Anything, "task-1").Return(expected).Once()

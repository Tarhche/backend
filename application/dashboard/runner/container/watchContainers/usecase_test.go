...
func TestUseCase_Handle(t *testing.T) {
	...
	t.Run("carries what became of a container back as it happens", func(t *testing.T) {
		t.Parallel()
		...
		stream := runnerMock.NewFakeContainerStream()

		authorizer.On("Authorize", mock.Anything, userUUID, permission.RunnerContainersIndex).Return(true, nil)
		runner.On("WatchContainers", mock.Anything).Return(stream, nil).Once()
		...
		require.NotNil(t, changed.Container)
		assert.Equal(t, "running", changed.Container.State)
		assert.Equal(t, "web-abcde."+ingressDomain, changed.Container.Endpoints[0].Host,
			"a container that has come up is shown with the address it answers on")

		assert.Equal(t, kindDeleted, deleted.Kind)
		assert.Equal(t, "two", deleted.UUID)
		assert.Nil(t, deleted.Container, "there is nothing left to describe about a container that is gone")
	})
	...
	t.Run("a client that walks away has its watch closed", func(t *testing.T) {
		...
		assert.Eventually(t, stream.IsClosed, 2*time.Second, 10*time.Millisecond,
			"nothing is left watching containers nobody is looking at")
	})

	t.Run("a watch is refused to somebody who may not list containers", func(t *testing.T) {
		t.Parallel()
		...
		authorizer.On("Authorize", mock.Anything, userUUID, permission.RunnerContainersIndex).Return(false, nil)
		authorizer.On("Authorize", mock.Anything, userUUID, permission.SelfRunnerContainersIndex).Return(false, nil)
		...
		runner.AssertNotCalled(t, "WatchContainers", mock.Anything)
	})
}

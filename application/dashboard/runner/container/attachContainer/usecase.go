...
func (uc *UseCase) Handle(ctx context.Context, data []byte) error {
	...
	// the token travels with it. This checked the permission, and the node
	// checks the one thing only it can: that the container is this person's.
	attachment, err := uc.runner.AttachContainer(ctx, runnerManager.Attach{

...
func TestNewEndpoints(t *testing.T) {
	...
	t.Run("the first port answers on the task's bare name", func(t *testing.T) {
		t.Parallel()
		...
		assert.Equal(t, uint(80), endpoints[0].TaskPort)
	...
	t.Run("a task with no name yet has no address to give", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("a task exposing nothing has no addresses", func(t *testing.T) {
		t.Parallel()
...
func TestNewTask(t *testing.T) {
	...
	assert.Equal(t, "Mahdi", presented.Owner.Name, "a task says who it belongs to")
...
func TestNewTask_deadline(t *testing.T) {
	t.Parallel()

	started := time.Now().Add(-time.Minute)
	ends := started.Add(10 * time.Minute)

	t.Run("a task that may only run for so long says when it runs out", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("a task that may run forever never expires", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("a task that is not running has nothing left to count down", func(t *testing.T) {
		t.Parallel()

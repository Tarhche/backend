...
// shows. The shapes live in one place because a task looks the same
...
// Task is one task, as the dashboard shows it.
type Task struct {
	...
	// MaxRetries is how many times a task that fails is asked for again
	...
	// Reason is why a task failed, when the runner can say so.
	Reason string `json:"reason,omitempty"`

	// Owner is who asked for this task.
	...
	// Deadline is when a task that is only allowed to run for so long
	// will be stopped. A task with no limit of its own has none.
	Deadline *time.Time `json:"deadline,omitempty"`
}

// Endpoint is one of a task's exposed ports, together with the hostname it
// answers on. The node and host port behind it stay inside the runner: a caller
// reaches a port by the task's own name.
...
// NewTask presents one task. The ingress domain is what its hostnames
// are built from, so the dashboard can link straight to a running task.
func NewTask(t task.Task, ingressDomain string, owners Owners) Task {
	return Task{
		UUID:          t.UUID,
		Name:          t.Name,
		Slug:          t.Slug,
...
// The node that made it sets it as it comes up, so a task that is not
...
// NewTasks presents a list of tasks.
func NewTasks(tasks []task.Task, ingressDomain string, owners Owners) []Task {
	items := make([]Task, len(tasks))
	for i := range tasks {
		items[i] = NewTask(tasks[i], ingressDomain, owners)
	}

	return items
}

// NewEndpoints builds the addresses a task's ports are served on.
//
// The first exposed port answers on the task's bare name, and every port
...
func NewEndpoints(t task.Task, ingressDomain string) []Endpoint {
	...
	for i, e := range t.Endpoints {
		// the runner reports only the ports it can actually reach, and keeps
		// how it reaches them to itself, so there is nothing here to filter on
		// beyond having a name to serve them under.
		if len(t.Slug) == 0 {
			continue
		}

		host := fmt.Sprintf("%s-%d.%s", t.Slug, e.TaskPort, ingressDomain)
		if i == 0 {

...
func TestUseCase_Handle(t *testing.T) {
	...
	t.Run("a request opens a watch on the asker's own tasks", func(t *testing.T) {
		t.Parallel()
		...
		assert.Equal(t, []string{requestID}, watchers.Tasks(ownerUUID))
		assert.Empty(t, watchers.Tasks("somebody-else"), "somebody else's task is not their news")
	})
	...
	t.Run("a client that walks away is no longer watching", func(t *testing.T) {
		...
		assert.Empty(t, watchers.Tasks(ownerUUID), "nothing is told to a watch nobody is behind")
	})
	...
	t.Run("a malformed request is dropped rather than redelivered", func(t *testing.T) {
		...
		assert.Empty(t, watchers.Tasks(ownerUUID))
	})
}

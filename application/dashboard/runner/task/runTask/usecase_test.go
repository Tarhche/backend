...
func TestUseCase_Execute(t *testing.T) {
	...
	t.Run("hands the specification to the runner as it was written", func(t *testing.T) {
		...
		assert.Equal(t, "http://nginx-xkfqz.runner.localhost:8021", response.Task.Endpoints[0].URL)
	})
	...
	t.Run("a request the rules refuse never reaches the runner", func(t *testing.T) {
		...
		runner.AssertNotCalled(t, "RunTask", mock.Anything, mock.Anything, mock.Anything)
	})
}

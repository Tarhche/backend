...
// WatchName is the subject a client opens a tasks watch on.
const WatchName = "runnerTasksWatch"

// UseCase registers the clients watching the tasks.
type UseCase struct {
...
func (uc *UseCase) Handle(ctx context.Context, data []byte) error {
	...
	uc.watchers.WatchTasks(request.ID, "")

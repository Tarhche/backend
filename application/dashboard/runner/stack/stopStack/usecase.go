...
// UseCase stops every service of a stack. The runner owns its lifecycle, so this passes the
...
func (uc *UseCase) Execute(ctx context.Context, request *Request) error {
	if err := uc.guard.OverStack(ctx, request.ActorUUID, permission.RunnerStacksManage, permission.SelfRunnerStacksManage, request.UUID); err != nil {
		return err
	}

...
func TestUseCase_Handle(t *testing.T) {
	...
	t.Run("a request opens a watch on every stack", func(t *testing.T) {
		t.Parallel()
		...
		assert.Equal(t, []string{requestID}, watchers.Stacks("somebody"))
		assert.Equal(t, []string{requestID}, watchers.Stacks(ownerUUID), "whosever the stack is")
	})
	...
	t.Run("a client that walks away is no longer watching", func(t *testing.T) {
		...
		assert.Empty(t, watchers.Stacks("somebody"), "nothing is told to a watch nobody is behind")
	})
	...
	t.Run("a malformed request is dropped rather than redelivered", func(t *testing.T) {
		...
		assert.Empty(t, watchers.Stacks("somebody"))
	})
}

...
func TestUseCase_Execute(t *testing.T) {
	...
	t.Run("updates an article the person wrote", func(t *testing.T) {
		t.Parallel()
		...
		articleRepository.On("GetByCorrelationUUIDAndLanguageAndAuthor", mock2.Anything, r.CorrelationUUID, r.LanguageCode, r.AuthorUUID).Once().Return(existing, nil)
	...
	t.Run("updating an article fails", func(t *testing.T) {
		...
		var (
			...
			existing = article.Article{UUID: "test-article-uuid", CorrelationUUID: r.CorrelationUUID, AuthorUUID: r.AuthorUUID, LanguageCode: "EN"}
			a        = article.Article{
				UUID:            existing.UUID,
				Cover:           r.Cover,
				Video:           r.Video,
	...
	t.Run("an article somebody else wrote is not there to update", func(t *testing.T) {
		t.Parallel()
		...
		articleRepository.On("GetByCorrelationUUIDAndLanguageAndAuthor", mock2.Anything, r.CorrelationUUID, r.LanguageCode, r.AuthorUUID).
		...
		articleRepository.AssertNotCalled(t, "Save", mock2.Anything, mock2.Anything)
	})
}

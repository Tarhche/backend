...
const (
	...
	// TaskOwnerLabelKey is whose container this is. The node that holds it is
	// the only thing that can say so without asking anybody: it is written on
	// the container when it is created, and read back off it when somebody
	// asks to be let in.
	TaskOwnerLabelKey = "task.owner"

...
func (r *MockStacksRepository) GetAllByOwner(ctx context.Context, ownerUUID string, offset uint, limit uint) ([]stack.Stack, error) {
	args := r.Mock.Called(ctx, ownerUUID, offset, limit)

	if items, ok := args.Get(0).([]stack.Stack); ok {
...
func (r *MockStacksRepository) CountByOwner(ctx context.Context, ownerUUID string) (uint, error) {
...
func (r *MockStacksRepository) GetOneByOwner(ctx context.Context, ownerUUID string, UUID string) (stack.Stack, error) {
	args := r.Mock.Called(ctx, ownerUUID, UUID)

...
import (
	...
	"github.com/khanzadimahdi/testproject/application/runner/controlplane/task/reconcile"
...
const (
	serveName string = "serve-runner-controlplane"
...
type ServeCommand struct {
	configs   *configs.RunnerControlPlane
...
func NewServeCommand() *ServeCommand {
	return &ServeCommand{configs: configs.NewRunnerControlPlane()}
}
...
// Providers returns the service providers required to serve the runner control plane.
func (c *ServeCommand) Providers() []provider.Provider {
	return []provider.Provider{
		providers.NewConfigsProvider(c.configs),
		providers.NewOpenTelemetryProvider("runner-controlplane", "runner-controlplane"),
		providers.NewProfilerProvider("runner-controlplane"),
		...
		runner.NewControlPlaneProvider(),
		c,
	}
}
...
func (c *ServeCommand) Boot(ctx context.Context, task provider.Container) error {
	...
	if err := task.Resolve(&c.logger, provider.WithParams("runner-controlplane")); err != nil {
		return err
	}
	...
	return task.Resolve(&c.consumers, provider.ResolveName(runner.ControlPlaneSubscribers))
}
...
// @version		1.0
// @description	Swagger/OpenAPI documentation for the runner control plane service.

...
func TestServe(t *testing.T) {
	t.Run("name", func(t *testing.T) {
		...
		want := "serve-runner-orchestrator"
		got := command.Name()
	...
	t.Run("usage", func(t *testing.T) {
		...
		want := "serve-runner-orchestrator [arguments]"
		got := command.Usage()
	...
	t.Run("configure the orchestrator name", func(t *testing.T) {
		testCases := []struct {
			name      string
			arguments []string
			env       string
			...
			{
				name:      "long flag",
				arguments: []string{"--name", "runner-orchestrator-01"},
				want:      "runner-orchestrator-01",
			},
			{
				name:      "short flag",
				arguments: []string{"-n", "runner-orchestrator-01"},
				want:      "runner-orchestrator-01",
			},
			{
				name: "environment variable",
				env:  "runner-orchestrator-01",
				want: "runner-orchestrator-01",
			},
			{
				name:      "the flag wins over the environment variable",
				arguments: []string{"--name", "runner-orchestrator-02"},
				env:       "runner-orchestrator-01",
				want:      "runner-orchestrator-02",
			},
		}
		...
		for _, testCase := range testCases {
			t.Run(testCase.name, func(t *testing.T) {
				if testCase.env != "" {
					t.Setenv("RUNNER_ORCHESTRATOR_NAME", testCase.env)
				}
	...
	t.Run("run", func(t *testing.T) {
		...
		// the background work a running orchestrator does. It is set here because
		...
		// point: the tunnel being down is not the orchestrator being down.
		tunnelOrchestrator, err := tunnel.NewAgent(
			consumerName,
			[]string{"127.0.0.1:1"},
		...
		command.tunnel = tunnelOrchestrator

		...
		<div :class="{ 'modal-dialog': modal, 'modal-lg': modal }">
			<div :class="{ 'modal-content': modal, card: !modal }">
				...
				<div :class="{ 'modal-body': modal, 'card-body': !modal }">
					<label :class="{ active: isSelected(file.uuid) }" class="brick tile-picker" v-for="(file, index) in data.items" :key="index" :style="{backgroundImage: `url('${useFilesUrlResolver().resolve(file.uuid)}')`}">
					...
					<p v-if="!data.pending && !data.error && data.items.length == 0" class="alert alert-info">هیچ فایلی وجود ندارد</p>
				</div>
				<div :class="{ 'modal-footer': modal, 'card-footer': !modal }">
					<label for="file-picker" type="button" class="btn btn-primary m-1">
						...
						<input accept="image/*" @change="uploadFile" @click="clearPickedFiles" class="visually-hidden" id="file-picker" type="file" name="file" multiple />
					</label>

...
The manager is a control plane: it schedules, reconciles and reaps, and it proxies a terminal or a live log to the node holding the container (as k8s does through its API server). It is **not** on the path of a container's own traffic. That is `serve-runner-ingress` (`:8021` locally), a service of its own reading tasks straight from mongo, so the manager can be down without a container becoming unreachable:

- **http** is routed by hostname — `<slug>.<domain>` for the lowest exposed port, `<slug>-<port>.<domain>` for a named one — and proxied to the node's published port.
- **anything else** (ssh, a database client) is carried whole on a port of its own from `RUNNER_INGRESS_PORT_RANGE`. The manager gives a container one of those ports per exposed port when it comes up; the ingress polls mongo and opens a tcp and udp listener for each. Docker publishes every exposed port for both protocols, so a container's port answers on one public port whichever it speaks.

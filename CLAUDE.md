...
**Firecracker** (`infrastructure/runner/firecracker` — read its README first, it carries the whole design) is split in two on purpose, and **nothing of it holds privilege on the host**. The **launcher** (`serve-runner-launcher`, `application/runner/launcher`, `domain/runner/machine`) starts each machine's firecracker and makes the bridges, taps and firewall it is plugged into, and nothing else. It is an ordinary container: `/dev/kvm` and `/dev/net/tun` are passed in, it has a handful of capabilities (`NET_ADMIN` among them) that reach no further than itself, and none of the host's namespaces, so the machines' networks live in its own network namespace and the host's firewall is never touched. Each machine runs as a user of its own (`RUNNER_MACHINE_FIRST_UID`/`RUNNER_MACHINE_UIDS`, `firecracker/vmm`), which owns its directory, its disk and its taps and nothing of any other machine's; a public network reaches the internet and nothing private or local (`firecracker/hostnet`). It takes orders on a unix socket made for the orchestrators' uid alone, and keeps no state of its own. The **orchestrator** holds no privilege either: it makes OCI images into read-only squashfs roots, boots each machine through its firecracker's API with firecracker-go-sdk, and talks to the **agent** — `cmd/runner-guest`, the machine's init — over its vsock. The two share `/var/lib/runner` at the same path on both sides (`firecracker/layout`). Machines are the launcher's processes: an orchestrator redeployed leaves them running and the next takes them back; a launcher recreated ends them, and their orchestrators start them again.

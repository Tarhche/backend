...
The manager is a control plane: it schedules, reconciles and reaps, and it proxies a terminal or a live log to the node holding the container (as k8s does through its API server). It is **not** on the path of a container's own traffic — **the nodes serve that themselves**, so the manager can be down without a container becoming unreachable:
...
- **http** is routed by hostname — `<slug>.<domain>` for the lowest exposed port, `<slug>-<port>.<domain>` for a named one — on each node's `RUNNER_INGRESS_PORT`. A node that is not holding the container proxies to the one that is, so a proxy in front may send a request to any node.
- **anything else** (ssh, a database client) is carried whole on a port out of that node's own `RUNNER_WORKER_PUBLIC_PORT_RANGE`: the node gives one to each of its containers' ports, writes them on the container as a label — which is what it reads them back from after a restart — and listens on them. Docker publishes every exposed port for both tcp and udp, so one public port carries whichever the container speaks, and the address is reported as `RUNNER_WORKER_PUBLIC_HOST:<port>`.

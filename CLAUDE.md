...
- **`/workers/{name}/...` → a worker's own HTTP API**, carried as a stream to its `api` service, path and all, websocket upgrades included. This is how the manager's `/api/tasks/{uuid}/attach` reaches the node holding a container (`RUNNER_INGRESS_URL`).
